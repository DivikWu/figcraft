...
- 🎨 **从创建到交付，全程覆盖** — 直接在 Figma 中创建 UI，搭配 [Figma 官方 MCP server](https://developers.figma.com/docs/figma-mcp-server/) 能力更强。画完即检查，质量问题当场修
- 🔍 **设计规范自动审查** — Token 绑定、颜色对比度、间距规范、组件健康度，一次扫描全覆盖
- 🔧 **检查+修复一步到位** — 36 条规则覆盖 Token 合规、WCAG、布局结构，一条命令批量自动修复
...
FigCraft 可以直接在 Figma 中创建 UI（frame、形状、文本、组件），也能搭配 [Figma 官方 MCP server](https://developers.figma.com/docs/figma-mcp-server/) 获得更丰富的创建能力。无论哪种方式，FigCraft 的质量工具都能帮你把关：

- 创建 UI 后，运行 `lint_fix_all` 检查并自动修复布局、Token、无障碍问题
...
- WCAG 无障碍（4）：对比度、点击区域、字号、行高
- 布局结构（16）：auto-layout 中的 fixed 元素、空容器、Spacer frame、嵌套深度、按钮结构、文本溢出、表单一致性、CTA 宽度不一致、子元素溢出父容器、HUG/STRETCH 矛盾、缺失 auto-layout、section spacing 塌缩、输入框结构、移动端尺寸、系统栏全出血、no-autolayout 强制检查
- 整屏质量（8）：header 碎片化、header 位置异常、交互元素根节点误分类、嵌套交互 shell、无效 screen shell、底部溢出、social/nav/stats 行拥挤
- 命名（1）：默认命名检查
- 组件（1）：组件绑定检查
