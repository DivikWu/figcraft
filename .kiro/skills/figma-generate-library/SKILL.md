...
```
Phase 0: DISCOVERY (always first — no execute_js writes yet)
  ...
  0c. Search subscribed libraries → use load_toolset("library") + list_library_components/variables for reusable assets
...
Phase 3: COMPONENTS (one at a time — never batch)
  For EACH component (in dependency order: atoms before molecules):
    ...
    3g. Validate: get_current_page (structure) + export_image (visual)
...
**Plugin API basics** (from figma-use skill — enforced here too):
...
2. **Inspect before creating** — run read-only `execute_js` to discover existing conventions. Match them.
...
12. **⛔ Verify after every step (non-negotiable)** — `get_current_page` to check structure, `export_image` to check visuals. Never execute multiple write `execute_js` calls without verification in between.
13. **NEVER parallelize `execute_js` calls** — Figma state mutations must be strictly sequential. Even if your tool supports parallel calls, never run two execute_js calls simultaneously.
...
15. **Use the helper scripts** — embed scripts from `scripts/` into your execute_js calls. Don't write 200-line inline scripts from scratch.
...
> **`getPluginData()` / `setPluginData()` are NOT supported in `execute_js`.** Use `getSharedPluginData()` / `setSharedPluginData()` instead (these ARE supported), or use name-based lookups and the state ledger (returned IDs).
...
**Resume protocol**: at session start or after context truncation, run a read-only `execute_js` to scan all pages, components, variables, and styles by name to reconstruct the `{key → id}` map. Then re-read the state file from disk if available.
...
## 5. Library Reuse — Decision Matrix

Search FIRST in Phase 0, then again immediately before each component creation. Use `load_toolset("library")` to enable library tools, then use `components(method: "list_library")` and library variable tools to discover reusable assets.
...
- ❌ Skipping library search before planning component creation
...
- ❌ Parallelizing execute_js calls (always sequential)
...
- ❌ Never execute multiple write `execute_js` calls without `export_image` or `get_current_page` verification in between
- ❌ Never run `lint_fix_all` on a large wrapper containing multiple screens/components (causes timeout — run per individual screen/component instead)
...
Reusable Plugin API helper functions. Embed in `execute_js` calls:
