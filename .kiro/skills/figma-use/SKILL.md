...
3a. `getPluginData()` / `setPluginData()` are **not supported** in `execute_js` — do not use them. Use `getSharedPluginData()` / `setSharedPluginData()` instead (these ARE supported), or track node IDs by returning them and passing them to subsequent calls.
4.  `console.log()` is NOT returned — use `return` for output
5.  **Work incrementally in small steps.** Break large operations into multiple `execute_js` calls. Validate after each step. This is the single most important practice for avoiding bugs.
...
**Page context resets between `execute_js` calls** — `figma.currentPage` starts on the first page each time.

### Switching pages

Use `await figma.setCurrentPageAsync(page)` to switch pages and load their content. The sync setter `figma.currentPage = page` **throws an error** in `execute_js` runtimes.
...
`figma.currentPage` resets to the **first page** at the start of each `execute_js` call. If your workflow spans multiple calls and targets a non-default page, call `await figma.setCurrentPageAsync(page)` at the start of each invocation.

You can call `execute_js` multiple times to incrementally build on the file state, or to retrieve information before writing another script. For example, write a script to get metadata about existing nodes, `return` that data, then use it in a subsequent script to modify those nodes.
...
`execute_js` works in **design mode** (editorType `"figma"`, the default). FigJam (`"figjam"`) has a different set of available node types — most design nodes are blocked there.
...
The most common cause of bugs is trying to do too much in a single `execute_js` call. **Work in small steps and validate after each one.**

### The pattern

1. **Inspect first.** Before creating anything, run a read-only `execute_js` to discover what already exists in the file — pages, components, variables, naming conventions. Match what's there.
...
4. **⛔ Verify after every step (non-negotiable)** — Use `get_current_page` to check structure, use `export_image` to check visuals. Never execute multiple write `execute_js` calls without verification in between.
...
Step 2: Create tokens/variables (if needed)
       → validate with get_current_page
Step 3: Create individual components
       → validate with get_current_page + export_image
Step 4: Compose layouts from component instances
       → validate with export_image
Step 5: Final verification
```

### What to validate at each step

| After... | Check with `get_current_page` | Check with `export_image` |
|---|---|---|
...
1. Call `get_current_page` to check structural correctness (hierarchy, counts, positions).
2. Call `export_image` to check visual correctness. Look closely for cropped/clipped text (line heights cutting off content) and overlapping elements — these are common and easy to miss.
...
Before submitting ANY `execute_js` call, verify:
...
|-----|-------------|----------------|
| [gotchas.md](references/gotchas.md) | Before any `execute_js` | Every known pitfall with WRONG/CORRECT code examples |
...
| [validation-and-recovery.md](references/validation-and-recovery.md) | Multi-step writes or error recovery | `get_current_page` vs `export_image` workflow, mandatory error recovery steps |
