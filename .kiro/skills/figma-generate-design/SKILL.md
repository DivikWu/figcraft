...
**Preferred: inspect existing screens first.** If the target file already contains screens using the same design system, inspect existing instances directly. A single `execute_js` call that walks an existing frame's instances gives you an exact, authoritative component map:
...
Only fall back to `load_toolset("library")` + `list_library_components` when the file has no existing screens to reference. When using it, **search broadly** — try multiple terms and synonyms (e.g., "button", "input", "nav", "card", "accordion", "header", "footer", "tag", "avatar", "toggle", "icon", etc.).
...
**Inspect existing screens first** (same as components). Or use `load_toolset("library")` + `list_library_variables`.

> **WARNING: Two different variable discovery methods — do not confuse them.**
>
> - `execute_js` with `figma.variables.getLocalVariableCollectionsAsync()` — returns **only local variables defined in the current file**. If this returns empty, it does **not** mean no variables exist. Remote/published library variables are invisible to this API.
> - `load_toolset("library")` + `list_library_variables` — searches across **all linked libraries**, including remote and published ones. This is the correct approach for discovering design system variables.
>
> **Never conclude "no variables exist" based solely on `getLocalVariableCollectionsAsync()` returning empty.** Always also use `list_library_variables` to check for library variables before deciding to create your own.

**Query strategy:** When searching library variables, run multiple short, simple queries in parallel rather than one compound query:
...
Use `load_toolset("styles")` + `scan_styles` or `list_library_styles` with terms like "heading", "body", "shadow", "elevation". Or inspect what an existing screen uses:
...
**Do NOT build sections as top-level page children and reparent them later** — moving nodes across `execute_js` calls with `appendChild()` silently fails and produces orphaned frames. Instead, create the wrapper first, then build each section directly inside it.

Create the page wrapper in its own `execute_js` call. Position it away from existing content and return its ID:
...
### Step 4: Build Each Section Inside the Wrapper (Strict Build → Verify Cycle)

**This is the most important step.** Build one section at a time, each in its own `execute_js` call. At the start of each script, fetch the wrapper by ID and append new content directly to it.

> ⚠️ **STRICT BUILD → VERIFY CYCLE (mandatory)**
>
> For each section, you MUST follow this exact sequence:
>
> 1. **Build**: Execute ONE `execute_js` call to build ONE section.
> 2. **⛔ Verify immediately (non-negotiable)**: After `execute_js` returns, you MUST call `export_image` to screenshot and verify the result. Never proceed without this step.
> 3. **Fix**: If issues are found, use targeted `execute_js` calls to fix — do NOT rebuild the entire section.
> 4. **Next section**: Only after the current section passes verification may you begin the next one.
>
> **NEVER execute multiple section `execute_js` calls without an `export_image` verification in between.**
> **NEVER build an entire screen in a single `execute_js` call.** The finer the section granularity, the higher the quality.
...
After each section, validate with `export_image` before moving on. Look closely for cropped/clipped text (line heights cutting off content) and overlapping elements — these are the most common issues and easy to miss at a glance.
...
After composing all sections, call `export_image` on the full page frame and compare against the source. Fix any issues with targeted `execute_js` calls — don't rebuild the entire screen.
...
**Run `lint_fix_all` per individual screen, not on a large wrapper containing multiple screens.** Linting a wrapper that contains many screens can cause timeouts. Target each screen frame by its node ID individually.

### Step 6: Updating an Existing Screen

When updating rather than creating from scratch:

1. Use `get_current_page` or `nodes(method: "get")` to inspect the existing screen structure.
...
3. For each section that needs changes:
   ...
   - Add new sections
4. Validate with `export_image` after each modification.
...
- **Work section by section.** Never build more than one major section per `execute_js` call.
- **Verify after every section.** Use `export_image` immediately after each `execute_js` — never skip this step.
...
- **Run `lint_fix_all` per individual screen.** Do not lint a wrapper containing multiple screens — it can cause timeouts.
