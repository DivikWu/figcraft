...
1. **`get_mode` 是必须的入口** — UI 创建工具（create_frame/create_text/create_svg）会在 bridge 层检查 get_mode 是否已调用。未调用直接报错。
2. **`get_mode` 返回 `_workflow`** — 包含设计决策 checklist、颜色/字体规则、创建步骤。AI 必须按 `_workflow` 执行。
3. **设计提案确认** — `_workflow.designPreflight` 要求 AI 在创建前向用户呈现设计方案并等待确认。
4. **`search_design_system` 按库状态自动分流** — 未选库跳过、本地模式只搜本地、选中库只搜该库。

```
get_mode → 读取 _workflow → 完成设计 checklist → 呈现方案给用户 → 等用户确认 → create_frame → 验证 → lint_fix_all
```

> 规则的 single source of truth 在 `packages/core-mcp/src/tools/logic/mode-logic.ts`，不在本文件。本文件只做引导。

### ⚠️ create_frame 的 Opinion Engine

> MCP 工具查询完整文档：`get_creation_guide(topic:"opinion-engine")`

`create_frame` 内置了 Opinion Engine（推断引擎），自动处理以下 Figma API 陷阱：
- **双轴 sizing 推断**：根据父容器上下文自动设置 layoutSizingHorizontal/Vertical，不会出现 100px 默认值问题
- **FILL 顺序**：内部先 appendChild 再设置 FILL，不会报错
- **冲突检测**：FILL + 显式 width 等矛盾参数会被拦截
- **失败清理**：创建失败时自动清理孤立节点
- **Token 自动绑定**：library 模式下自动匹配变量和样式
- **dryRun 预验证**：text overflow、invisible frame、fontSize < 12 等问题在创建前预警

> `execute_js` 已移至 `debug` 工具集，默认不可用。所有 UI 创建必须使用声明式工具。

## Multi-Screen Flow 创建规则

> MCP 工具查询：`get_creation_guide(topic:"multi-screen")`

多屏流程（登录注册、引导、结账等）**必须使用 `create_frame` + `children` 构建**。
...
create_frame 的 Opinion Engine 会自动处理：sizing 推断、FILL 顺序、冲突检测、跨层级校验、失败清理。不需要手动处理这些 Figma API 细节。

### 关键规则

- Screen 使用 `primaryAxisAlignItems: "SPACE_BETWEEN"` 分布上下内容（仅当内容足够填满屏幕时；稀疏内容屏用 `"MIN"`）
- Shadow 元素的所有祖先容器必须 `clipsContent: false`
- 不确定参数时用 `dryRun: true` 预览推断结果
...
### 基础
- `ping` — 测试连通性
- `search_design_system` — 搜索设计系统资产（组件、变量、样式），跨所有已订阅 library，支持模糊匹配和相关性排序
...
> FigCraft 提供完整的纯声明式 UI 创建（`create_frame`、`create_text`、`create_svg`）、文本范围样式（`text(method: "set_range")`）、节点分组（`group_nodes`）、设计系统搜索、审查、lint（37 规则 + fixCall）、审计和 token 同步能力。`create_frame` 和 `create_text` 支持 `items[]` 批量创建。`create_frame` 支持 `dryRun` 预览模式（歧义参数时不创建节点，返回 correctedPayload）。`execute_js` 仅在 `debug` 工具集中可用（`load_toolset("debug")`），默认不暴露给 AI。
