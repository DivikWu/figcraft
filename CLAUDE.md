...
1. **`get_mode` 是必须的入口** — UI 创建工具（create_frame/create_text/create_svg/execute_js）会在 bridge 层检查 get_mode 是否已调用。未调用直接报错。
...
### ⚠️ 为什么必须用 create_frame 而非 execute_js
...
使用 execute_js 会**绕过所有保护**，导致需要手动处理这些问题。
...
> FigCraft 提供完整的 UI 创建（`create_frame`、`create_text`、`create_svg`、`execute_js`）、设计系统搜索、审查、lint（37 规则 + fixCall）、审计和 token 同步能力。`create_frame` 和 `create_text` 支持 `items[]` 批量创建。`create_frame` 支持 `dryRun` 预览模式（歧义参数时不创建节点，返回 correctedPayload）。可独立使用，也可与 Figma 官方 MCP 互补。
