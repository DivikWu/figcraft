...
export function registerStagingTools(server: McpServer, bridge: Bridge): void {
  ...
  server.tool(
    'discard_changes',
    'Remove staged nodes from the canvas entirely. ' + 'If no nodeIds provided, discards all currently staged nodes.',
    {
  ...
  server.tool('list_staged', 'List all currently staged nodes.', {}, async () => {
