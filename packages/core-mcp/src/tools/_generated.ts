...
export function registerGeneratedTools(
  ...
  if (shouldRegisterGeneratedTool(include, 'create_component')) {
    ...
    "Create a component with layout, fill, stroke, inline children, and component properties in one call. Delegates to create_frame internally — all Opinion Engine inferences apply automatically. TEXT properties auto-bind to child text nodes via componentPropertyName. Icon children ({type:\"icon\", icon:\"lucide:...\"}) are resolved to SVG by the harness before Plugin dispatch, same as create_frame. Accepts all create_frame params via passthrough — only common params listed here.\nOmit x/y to auto-position below existing content. For fixed-height components (buttons), pass height + layoutSizingVertical:\"FIXED\" to prevent Opinion Engine HUG override. Use fillVariableId (from designContext.defaults.*.id) for direct variable binding — zero name resolution. Harness auto-injects section parentId — omit parentId to auto-place inside last created section.\nBatch mode (STRONGLY preferred for variant matrices ≥ 8): pass `items: [...]` with up to 20 component param objects in one call. Each item gets inline `properties: [...]` and inline `children: [...]` (including {type:\"icon\"}). Follow with create_component_set to combine them into a variant set. The clone-and-modify alternative costs 3-5× more tool calls and trips Opinion Engine re-inference edge cases.\nWorkflow (single): create_section → create_component(fillVariableId:...) → bind_component_property → create_component_set. Workflow (batch): create_section → create_component({items:[...]}) → create_component_set.",
    {
      ...
      contentWrapper: z.boolean().optional().describe("Wrap inline children in a transparent \"Content\" frame (default: true). The Content frame inherits layoutMode, alignment, and itemSpacing from the component. Pass false to disable (e.g. for simple components without structural variants)."),
      ...
      fillVariableId: z.string().optional().describe("PREFERRED — bind fill to a variable by ID (e.g. \"VariableID:123:456\" from get_design_context defaults.*.id). Zero name resolution, zero fragility. Use this whenever you have the ID from a prior get_design_context call."),
      fillVariableName: z.string().optional().describe("Bind fill to a color variable by name (e.g. 'bg/primary'). FALLBACK — use fillVariableId instead when you have the ID. Name lookup tolerates case and separator variants (/, ., -, _) but is still fragile across libraries."),
      ...
      strokeVariableId: z.string().optional().describe("PREFERRED — bind stroke to a variable by ID. Zero name resolution, zero fragility. Use this whenever you have the ID from a prior get_design_context call."),
      strokeVariableName: z.string().optional().describe("Bind stroke to a color variable by name. FALLBACK — use strokeVariableId when the ID is available."),
...
export const styles_epEndpointSchema = {
      ...
      textDecoration: z.string().optional().describe("Text decoration: NONE, UNDERLINE, STRIKETHROUGH"),
      textCase: z.string().optional().describe("Text case: ORIGINAL, UPPER, LOWER, TITLE, SMALL_CAPS, SMALL_CAPS_FORCED"),
      bindings: z.array(z.record(z.unknown())).optional().describe("Bind variables to text style fields. Array of {field, variableId}. Fields: fontFamily, fontSize, fontStyle, fontWeight, letterSpacing, lineHeight, paragraphSpacing, paragraphIndent"),
