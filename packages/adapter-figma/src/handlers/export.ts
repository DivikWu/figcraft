...
export function registerExportHandlers(): void {
  registerHandler('export_image', async (params) => {
    ...
    const format = ((params.format as string) ?? 'PNG').toUpperCase() as 'PNG' | 'SVG' | 'PDF' | 'JPG';
    const scale = (params.scale as number) ?? 2;

    const node = await findNodeByIdAsync(nodeId);
    assertHandler(node && 'exportAsync' in node, `Node not found or not exportable: ${nodeId}`, 'NOT_FOUND');
