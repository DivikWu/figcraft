...
import { STORAGE_KEYS, PLUGIN_DATA_KEYS } from '../constants.js';
...
interface LibraryCtx {
  ...
  /** Structured telemetry for benchmark/debug consumption. */
  debugStats: {
  ...
  /** Structural creation errors detected before node creation. */
...
function applyNodeRole(node: BaseNode, role: string | undefined): void {
  if (!role || !('setPluginData' in node)) return;
  node.setPluginData(PLUGIN_DATA_KEYS.ROLE, role);
}
...
async function createNodeFromSpec(spec: NodeSpec, parentNode: BaseNode | undefined, ctx: LibraryCtx): Promise<SceneNode> {
  ...
  if (spec.type === 'frame') {
    ...
    for (const structuralError of inference.structuralErrors) {
      ctx.hints.push({ type: 'error', message: structuralError });
      ctx.structuralErrors.push(structuralError);
    }
    ctx.debugStats.heuristicHits += inference.telemetry.heuristicHits;
    ctx.debugStats.heuristicSkips += inference.telemetry.heuristicSkips;
    ctx.debugStats.ambiguousFixCount += inference.telemetry.ambiguousFixCount;
    ctx.debugStats.rootRoleGuesses += inference.telemetry.rootRoleGuesses;
    ctx.debugStats.interactiveGuessConflicts += inference.telemetry.interactiveGuessConflicts;
    if (inference.structuralErrors.length > 0) {
      throw new Error(inference.structuralErrors.join(' | '));
    }
    ...
    applyNodeRole(ellipse, spec.role);
    ...
    applyNodeRole(vectorNode, spec.role);
    ...
    applyNodeRole(instance, spec.role);
...
registerHandler('create_document', async (params) => {
  ...
  result.debugStats = { ...ctx.debugStats };
  ...
  if (ctx.structuralErrors.length > 0) {
    result.structuralErrors = [...new Set(ctx.structuralErrors)];
  }
