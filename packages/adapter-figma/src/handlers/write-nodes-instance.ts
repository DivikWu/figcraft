...
import { applySizingOverrides, setLayoutSizing, resolveComponent } from '../utils/figma-compat.js';
...
registerHandler('create_component_from_node', async (params) => {
  ...
  assertHandler(node.type === 'FRAME' || node.type === 'GROUP' || node.type === 'RECTANGLE',
...
registerHandler('create_instances', async (params) => {
  ...
  for (const item of items) {
    try {
      ...
      results.push({ id: String(item.componentId ?? '?'), ok: false, error: err instanceof Error ? err.message : String(err) });
    }
  }
  return { results };
});
...
registerHandler('text_scan', async (params) => {
  ...
  const textNodes: Array<{
    ...
    fontSize: number | typeof figma.mixed; fontFamily: string;
    path?: string;
  }> = [];
