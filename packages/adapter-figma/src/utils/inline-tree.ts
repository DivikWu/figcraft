 ...
 *
 * Inline-tree structure inference engine — auto-fixes common structural mistakes
...
export interface InferenceResult {
  ...
  ambiguous: string[];
  /** Structural errors that should not be silently papered over. */
  ...
  /** Debug counters for heuristic and ambiguity behavior. */
  telemetry: {
    heuristicHits: number;
    heuristicSkips: number;
    ambiguousFixCount: number;
    rootRoleGuesses: number;
    interactiveGuessConflicts: number;
  };
}
...
const BUTTON_ROLE_SET = new Set(['button', 'cta', 'icon_button']);
const INPUT_ROLE_SET = new Set(['input', 'field', 'search', 'search_bar', 'searchbar']);
const INTERACTIVE_ROLE_SET = new Set([...BUTTON_ROLE_SET, ...INPUT_ROLE_SET]);
const CONTAINER_ROLE_SET = new Set([
  'screen',
  'page',
  'section',
  'header',
...
const SCREEN_NAME_BLOCKLIST_RE = /welcome|sign.?in|sign.?up|forgot\s+password|create\s+account|onboarding|settings|profile|dashboard|checkout|pricing|empty\s+state/i;
...
function getNodeRole(spec: NodeSpec): string | undefined {
  return normalizeRole(spec.role);
}

function isButtonRole(role?: string): boolean {
  return !!role && BUTTON_ROLE_SET.has(role);
}

function isInputRole(role?: string): boolean {
  return !!role && INPUT_ROLE_SET.has(role);
}

function isInteractiveRole(role?: string): boolean {
  return !!role && INTERACTIVE_ROLE_SET.has(role);
}

function isContainerRole(role?: string): boolean {
  return !!role && CONTAINER_ROLE_SET.has(role);
}

function hasDirectTextChild(spec: NodeSpec): boolean {
  return !!spec.children?.some((child) => child.type === 'text');
}

function countFrameChildren(spec: NodeSpec): number {
  return spec.children?.filter((child) => child.type === 'frame' || child.type === 'instance').length ?? 0;
}

function isLikelySectionContainer(spec: NodeSpec, role?: string): boolean {
  if (isContainerRole(role)) return true;
  const childCount = spec.children?.length ?? 0;
  if (childCount >= 3) return true;
  if (countFrameChildren(spec) >= 2) return true;
  return false;
}

function hasButtonLikeStructure(spec: NodeSpec): boolean {
  const children = spec.children ?? [];
  if (children.length === 0 || children.length > 3) return false;
  if (!hasDirectTextChild(spec)) return false;
  const nestedFrames = children.filter((child) => child.type === 'frame' && (child.children?.length ?? 0) > 1);
  if (nestedFrames.length > 0) return false;
  return true;
}

function hasInputLikeStructure(spec: NodeSpec): boolean {
  const children = spec.children ?? [];
  if (children.length === 0 || children.length > 3) return false;
  if (!hasDirectTextChild(spec)) return false;
  const nestedFrames = children.filter((child) => child.type === 'frame');
  return nestedFrames.length === 0;
}

function isInputLikeContext(parentCtx?: ParentCtx): boolean {
  if (!parentCtx) return false;
  const parentRole = normalizeRole(parentCtx.role);
  return isInputRole(parentRole) || looksLikeInput(parentCtx.name ?? '');
}

function isInteractiveLikeContext(parentCtx?: ParentCtx): boolean {
  if (!parentCtx) return false;
  const parentRole = normalizeRole(parentCtx.role);
  return isInteractiveRole(parentRole) ||
    looksLikeButton(parentCtx.name ?? '') ||
    looksLikeInput(parentCtx.name ?? '');
}
...
function looksLikeScreenRoot(spec: NodeSpec, name: string, role?: string): boolean {
  if (isContainerRole(role)) return true;
  if (SCREEN_NAME_BLOCKLIST_RE.test(name)) return true;
  if (isLikelySectionContainer(spec, role)) return true;
  const width = readDimension(spec.props?.width);
  const height = readDimension(spec.props?.height);
  return (width != null && width >= 360) && (height != null && height >= 640);
}

interface HeuristicDecision {
  apply: boolean;
  explicitRole: boolean;
  interactiveName: boolean;
  blockedReason?: 'container-role' | 'container-structure' | 'screen-root' | 'parent-interactive' | 'root-interactive';
}

function getButtonHeuristicDecision(spec: NodeSpec, name: string, parentCtx?: ParentCtx): HeuristicDecision {
  const role = getNodeRole(spec);
  if (isButtonRole(role)) {
    return { apply: true, explicitRole: true, interactiveName: looksLikeButton(name) };
  }
  if (isContainerRole(role)) {
    return { apply: false, explicitRole: false, interactiveName: looksLikeButton(name), blockedReason: 'container-role' };
  }
  const buttonLike = hasButtonLikeStructure(spec);
  const screenLike = looksLikeScreenRoot(spec, name, role);
  const containerLike = isLikelySectionContainer(spec, role);
  const interactiveName = looksLikeButton(name);
  if (containerLike) {
    return { apply: false, explicitRole: false, interactiveName, blockedReason: 'container-structure' };
  }
  if (parentCtx == null && screenLike && !buttonLike) {
    return { apply: false, explicitRole: false, interactiveName, blockedReason: 'screen-root' };
  }
  if (parentCtx != null && isInteractiveLikeContext(parentCtx)) {
    return { apply: false, explicitRole: false, interactiveName, blockedReason: 'parent-interactive' };
  }
  return { apply: interactiveName && buttonLike, explicitRole: false, interactiveName };
}

function getInputHeuristicDecision(spec: NodeSpec, name: string, parentCtx?: ParentCtx): HeuristicDecision {
  const role = getNodeRole(spec);
  if (isInputRole(role)) {
    return { apply: true, explicitRole: true, interactiveName: looksLikeInput(name) };
  }
  if (isContainerRole(role)) {
    return { apply: false, explicitRole: false, interactiveName: looksLikeInput(name), blockedReason: 'container-role' };
  }
  const screenLike = looksLikeScreenRoot(spec, name, role);
  const containerLike = isLikelySectionContainer(spec, role);
  const interactiveName = looksLikeInput(name);
  if (containerLike) {
    return { apply: false, explicitRole: false, interactiveName, blockedReason: 'container-structure' };
  }
  if (parentCtx == null && screenLike) {
    return { apply: false, explicitRole: false, interactiveName, blockedReason: 'screen-root' };
  }
  if (isInputLikeContext(parentCtx)) {
    return { apply: false, explicitRole: false, interactiveName, blockedReason: 'parent-interactive' };
  }
  return { apply: interactiveName && hasInputLikeStructure(spec), explicitRole: false, interactiveName };
}
...
export function inferStructure(spec: NodeSpec, parentCtx?: ParentCtx): InferenceResult {
  ...
  const screenLikeRoot = parentCtx == null && looksLikeScreenRoot(spec, name, role);
  if (parentCtx == null && !role && screenLikeRoot) {
    result.telemetry.rootRoleGuesses++;
  }
  ...
  const pushFix = (msg: string, confidence: 'deterministic' | 'ambiguous' = 'deterministic') => {
    ...
    if (confidence === 'ambiguous') result.telemetry.ambiguousFixCount++;
  };

  const pushStructuralError = (msg: string) => {
    result.structuralErrors.push(msg);
  };

  const buttonDecision = hasChildren ? getButtonHeuristicDecision(spec, name, parentCtx) : null;
  const inputDecision = hasChildren ? getInputHeuristicDecision(spec, name, parentCtx) : null;
  for (const decision of [buttonDecision, inputDecision]) {
    if (!decision || decision.explicitRole) continue;
    if (decision.apply) {
      result.telemetry.heuristicHits++;
      continue;
    }
    if (decision.interactiveName) {
      result.telemetry.heuristicSkips++;
      result.telemetry.interactiveGuessConflicts++;
    }
  }

  if (screenLikeRoot && isInteractiveRole(role)) {
    pushStructuralError(
      `"${name}": root frame looks like a screen but is marked as interactive (${role}). Use role="screen" and move button/input semantics to child nodes.`,
    );
  }
  if (screenLikeRoot && ((buttonDecision?.apply && !buttonDecision.explicitRole) || (inputDecision?.apply && !inputDecision.explicitRole))) {
    pushStructuralError(
      `"${name}": root frame looks like a screen but also matches an interactive heuristic. Root screen containers must not be inferred as button/input shells.`,
    );
  }
  if (parentCtx != null && isInteractiveLikeContext(parentCtx) &&
    ((buttonDecision?.apply === true) || (inputDecision?.apply === true))) {
    pushStructuralError(
      `"${name}": interactive shell nested inside interactive parent "${parentCtx.name ?? 'parent'}". Collapse duplicate button/input wrappers before creation.`,
    );
  }
  ...
  if (hasChildren && buttonDecision?.apply) {
    if (!spec.props) spec.props = {};
  ...
  if (hasChildren && inputDecision?.apply) {
    if (!spec.props) spec.props = {};
    if (!spec.props.autoLayout) {
      spec.props.autoLayout = true;
      spec.props.layoutDirection = spec.props.layoutDirection ?? 'HORIZONTAL';
  ...
  if (hasChildren) {
    const thisCtx: ParentCtx = {
      ...
      depth: parentCtx ? parentCtx.depth + 1 : 0,
      role,
      name,
    };
    ...
    for (const child of spec.children!) {
      if (child.type === 'frame') {
        ...
        result.structuralErrors.push(...childResult.structuralErrors);
        result.annotatedFixes.push(...childResult.annotatedFixes);
        result.correctedNodes.push(...childResult.correctedNodes);
        result.telemetry.heuristicHits += childResult.telemetry.heuristicHits;
        result.telemetry.heuristicSkips += childResult.telemetry.heuristicSkips;
        result.telemetry.ambiguousFixCount += childResult.telemetry.ambiguousFixCount;
        result.telemetry.rootRoleGuesses += childResult.telemetry.rootRoleGuesses;
        result.telemetry.interactiveGuessConflicts += childResult.telemetry.interactiveGuessConflicts;
      }
    }
  }
 ...
 *
 * Rules (axis-aware sizing inference):
...
export function inferChildSizing(
  ...
  // Deep axis-aware sizing inference for frame children
...
/**
 * Node-type-aware alias key sets for text and non-text nodes.
...
const ALIAS_MAP: Record<string, { target: string; transform?: (v: unknown) => unknown }> = {
  ...
  // Variable/Style name aliases → structured object format
...
export function checkOverlappingSiblingsPostCreation(
  ...
  for (const child of children) {
    // Use Math.round for sub-pixel tolerance
