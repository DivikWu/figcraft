 ...
 * shadow, or rounded corners. Section-level library components (Product List,
 * quick access links, etc.) often ship a thin COMPONENT root with padding = 0
 * and push all visuals into inner Header/Content wrappers, so peeking one
 * level catches that common case without flagging truly empty stacks.
 */
...
export const sectionSpacingCollapseRule: LintRule = {
  ...
  check(node: AbstractNode, ctx: LintContext): LintViolation[] {
    ...
    // effects, radius, or just section-sized height), visual rhythm is supplied
    // by the cards themselves — parent itemSpacing 8 is fine. Flagging would push
    // a cosmetic fix that double-pads the layout.
    ...
    return [
      {
        ...
        suggestion: tr(
          ctx.lang,
          `"${node.name}" packs major sections too tightly. Increase itemSpacing to restore a clearer vertical rhythm.`,
          `「${node.name}」主要区块间距过紧。请增加 itemSpacing 以恢复清晰的纵向节奏。`,
        ),
        autoFixable: true,
        fixData: {
          fix: 'item-spacing',
