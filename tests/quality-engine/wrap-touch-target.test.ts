...
// ─── word-boundary regex rejects substring false matches ───

describe('wcag-target-size name pattern precision', () => {
  it('ignores "Tabs - Secondary Tabs / Light" (layout wrapper, not a tab)', () => {
    // Word boundary on /\btab\b/ would still match "Tabs" (plural). That's fine
    // for declared-interactive fallback — Tabs collections are interactive too.
    // The real protection is: this wrapper is typically FRAME with large size,
    // so even if matched it won't fail the 24×24 check. Spot-check a non-match:
    const node = makeNode({ name: 'Table Row', type: 'FRAME', width: 10, height: 10 });
    expect(wcagTargetSizeRule.check(node, emptyCtx)).toHaveLength(0);
  });

  it('ignores "Touchable-ish" substrings that are not whole words', () => {
