...
FigCraft 的核心价值是让 AI 遵循设计规范做设计。项目有六类资产会随时间积累、复利增长，互相增强。

## 资产概览

| 资产 | 当前规模 | 复利效应 | 增长方式 |
|------|---------|---------|---------|
| **Skills** | 11 个 | AI 做得更好 | 新增/优化 skill |
| **Lint 规则** | 38 条（58% autoFix） | 质量底线自动抬高 | 新增规则 + autoFix |
| **Opinion Engine** | 10 项推断 + 13 项纠错 | 用户配置越来越少 | 新增智能默认值 |
| **UI 模板** | 9 种 × 3 tone = 27 变体 | 创建速度越来越快 | 新增模板 |
| **MCP 工具** | 97 个（28 core + 69 optional） | AI 能做的事越来越多 | 新增工具 |
| **测试套件** | 42 文件 / 727 测试 | 迭代信心越来越强 | 跟随功能新增 |
...
|------|--------|-------------|------|
| Layout（布局结构） | 19 | 13 (68%) | 50% |
| Token（规范合规） | 6 | 6 (100%) | 16% |
| WCAG（无障碍） | 5 | 3 (60%) | 13% |
| Naming（命名） | 2 | 0 (0%) | 5% |
| Component（组件） | 1 | 0 (0%) | 3% |

**总体 autoFix 率：58%**（22/38）
...
| P1 | **间距节奏推断** — 基于层级自动选择间距倍数（8/16/24/32） | 支撑 8dp 系统 |
...
|------|---------|--------|---------|
| 核心 | core | 28 | 始终加载 |
| 可选 | components-advanced | 13 | load_toolset |
| 可选 | tokens | 12 | load_toolset |
| 可选 | library | 8 | load_toolset |
| 可选 | shapes-vectors | 7 | load_toolset |
| 可选 | lint | 6 | load_toolset |
| 可选 | variables | 5 | load_toolset |
| 可选 | prototype | 5 | load_toolset |
| 可选 | staging | 4 | load_toolset |
| 可选 | pages | 3 | load_toolset |
| 可选 | styles | 3 | load_toolset |
| 可选 | auth | 3 | load_toolset |
| 可选 | annotations | 2 | load_toolset |
...
| 包 | 测试文件 | 覆盖范围 |
|----|---------|---------|
| quality-engine | 10 | lint 规则正确性、autoFix 行为 |
| core-mcp | 11 | 工具注册、endpoint dispatch、schema 合约 |
| adapter-figma | 8 | Plugin handler 逻辑、颜色转换、布局计算 |
| contracts | 4 | monorepo 结构、公共接口、向后兼容 |
| 其他 | 9 | relay、benchmark、version |
