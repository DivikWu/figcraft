...
| 质量保证 | 无内置 lint | 38 条 lint 规则 + 自动修复 + 创建后自检 |
| 设计规范 | 通过 `create_design_system_rules` 生成规则文件 | 运行时双模式（Library / Spec）+ 三层设计规则 |
| 工具数量 | 15 个工具 | 111+ 个工具（31 核心 + 13 可选工具集 + 5 资源端点） |
...
| `generate_figma_design` | 捕获实时 Web UI 转为 Figma 设计图层（remote only） | 无替代。FigCraft 的 `figma-generate-design` skill 可编排此工具作为视觉参考 |
...
| `figma-generate-design` | `generate_figma_design`（可选）+ `search_design_system` + `get_screenshot` | 从代码/描述构建 Figma 屏幕 |
...
FigCraft 通过本地 Plugin + WebSocket Relay 连接 Figma，**不需要 OAuth**。只要 Figma 桌面应用中打开了 FigCraft 插件，`ping` 通了就能使用全部 111 个原生工具。
...
| `figma-generate-design` skill | ⚠️ 部分受影响（核心流程可用，但 `generate_figma_design` 视觉参考不可用） |
