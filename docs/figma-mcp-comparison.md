...
|----------------|---------------------|------|
| `figcraft-implement-design` | `get_design_context` + `get_screenshot` + `get_metadata` | 从 Figma 设计生成代码 |
| `figcraft-code-connect` | `get_code_connect_suggestions` + `send_code_connect_mappings` | 建立组件-代码映射 |
| `figcraft-create-new-file` | `create_new_file` + `whoami` | 创建新 Figma/FigJam 文件 |
| `figcraft-create-design-system-rules` | `create_design_system_rules` | 生成项目级设计系统规则 |
...
| 从 Figma 设计生成代码 | 官方 MCP（`get_design_context`）或 FigCraft skill `figcraft-implement-design`（自动编排官方工具） |
| Token 同步（DTCG ↔ Figma） | FigCraft（`sync_tokens` / `diff_tokens`） |
| 设计质量检查 | FigCraft（`lint_fix_all` / `verify_design`） |
| Code Connect 映射 | 官方 MCP 或 FigCraft skill `figcraft-code-connect`（自动编排官方工具） |
...
- 但通过 `figcraft-create-new-file` skill 可以创建 FigJam 文件（调用官方 MCP 的 `create_new_file`）
...
| `figcraft-generate-library` skill | ✅ 不受影响（纯 FigCraft 工具） |
| `figcraft-implement-design` skill | ❌ 无法工作（依赖 `get_design_context` + `get_screenshot`） |
| `figcraft-code-connect` skill | ❌ 无法工作（依赖 Code Connect 工具） |
| `figcraft-create-new-file` skill | ❌ 无法工作（依赖 `create_new_file` + `whoami`） |
| `figcraft-create-design-system-rules` skill | ❌ 无法工作（依赖 `create_design_system_rules`） |
